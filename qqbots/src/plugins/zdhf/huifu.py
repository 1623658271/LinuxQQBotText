...
weather = on_keyword({'emo', 'wy'}, block=True, priority=4)
cici = on_message(priority=5)
qh = on_regex(pattern=r"^qh$", block=True, priority=4)
            ...
            if msg == '终止对话':
                ...
                try:
                    content = contentJson['result']['intents'][0]['outputs'][1]['property']['text'] + '[' + \
                              contentJson['result']['intents'][0]['outputs'][1]['property'].get('emotion', 'null') + ']'
                except:
                    content = '我不理解'
                #if more:
...
#    try:
#        content = content_json['result']['intents'][0]['result']['text']
#    except:
#        content = '出错了,当前没有对这种service意图进行判断哦'
#    return content
...
async def get_emo() -> str:
    with open("/var/qqbot/qqbots/src/plugins/emo.json", 'r', encoding='utf8') as fp:
...
async def get_emo2() -> str:
    with open("/var/qqbot/qqbots/src/plugins/emo2.json", 'r', encoding='utf8') as fp:
        json_data = json.load(fp)
    a = random.randint(0, 199)
    name = json_data['data'][a]['songName']
    b = random.randint(1, 14)
